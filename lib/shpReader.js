...
_shp = {
   ...
   */
  getRecord: function(shp, number) {
    var records
      ;

    // Extract all the records.
    records = _shp._extractRecords(shp.db, shp.header);
    ...
    if (number % 1 !== 0 || number < 1)
      /* istanbul ignore next */
      throw new Error('This record number "' + number + '" do not match!');

    // Return the requested record or null if it is out of range.
    if (records[number - 1] !== undefined)
   ...
   * @returns {Array}   The point coordinates,
   */
  _retrievePoint: function(db, offset) {
    return [
   ...
   * @param {Buffer}    The shp database,
   * @param {Number}    The position of the polyline/polygon in the db,
   * @returns {Array}   The polyline/polygon coordinates,
   */
  _retrievePolyLine: function(db, offset) {
    var polyline
      , P0
      , first
      , last
    ...
    P0 = offset + RECORD_HEAD + 44 + (4 * polyline.numParts);
   ...
   */
  _extractRecords: function(db, header) {
    var offset = RECORD_START
      , record = []
      , i
      ;

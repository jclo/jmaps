/* global  */
/* eslint one-var: 0, no-underscore-dangle: 0, no-param-reassign: 0, dot-notation: 0,
  semi-style: 0 */
/* eslint max-len: [1, 135, 2], no-buffer-constructor: 0, no-control-regex: 0 */
...
const _dbf = {
  ...
  getRecord(dbf, number) {
    ...
    if (number === undefined) {
      ...
      for (i = 0; i < dbf.header.numberOfRecords; i++) {
   ...
   */
  _retrieveFieldDescriptorArray(db, header) {
    const field  = []
   ...
   */
  _retrieveRecord(db, header, fieldDescriptorArray, number) {
    const record = {};
    let buffer

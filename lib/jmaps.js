/* global  */
/* eslint one-var: 0, no-underscore-dangle: 0, no-param-reassign: 0, semi-style: 0 */
/* eslint max-len: [1, 145, 2] */
...
// -- Local modules
const _dbf = require('./dbfReader.js')
...
JMAPS.prototype = {
  ...
  getFeature(recordNumber) {
    ...
    if (typeof recordNumber !== 'number' || recordNumber % 1 !== 0 || recordNumber <= 0 || recordNumber > this._dbf.header.numberOfRecords) {
      throw new Error(`Record Number "${recordNumber}" does not match!`);
    }

    // Retrieve the associated data from the shp db.
    const shprecord = _shp.getRecord(this._shp, recordNumber);
  ...
  transform(geojson, options) {
    ...
    if (!options.projection) { options.projection = 'none'; }
    ...
    if (geojson.type === 'Feature') {
      ...
      // Replace λ and φ coordinates by x, y plane coordinates to
      // GeoJSON object.
  ...
  toSVG(fsw, json) {
    ...
    if ((fsw === undefined || typeof fsw !== 'object') || (json === undefined || typeof json !== 'object')) {
      throw new Error('This method expects two objects as arguments!');
    }

    // Check if the file stream has the method write.
    if ('write' in fsw === false) {
      throw new Error('The file stream "fsw" is not a file stream or is not writable!');
    }
    ...
    fsw.write('<!-- Made with Natural Earth. Free vector and raster map data @ naturalearthdata.com. -->\n');
    fsw.write('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\n');
    fsw.write('  <g transform="translate(0, 0) scale(1, 1)">\n');

    // For a Feature.
    if (json.type === 'Feature') {
      d = _svg.getSVGPath(json.geometry.coordinates);
      // fsw.write('    <path id="" class="land" d="' + d + '"></path>\n');
      fsw.write(`    <path id="" class="land" d="${d}"></path>\n`);
    }
